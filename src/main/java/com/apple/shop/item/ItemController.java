...
public class ItemController { // JPA 사용1. repository  만들기 -> ItemRepository  등록
  ...
  // 다른 클래스 ItemDBManager, itemService도 생성해서 주입 (싱글톤 패턴)<- 의존성 주입
  // new를 밖에서 뽑아서 주입해줌  -> new 키워드 없앰 -> 의존성이 줄어듬 (클래스간 커플링(간섭) 줄임)
  @Autowired
  public ItemController(ItemDBManager dbManager, ItemService itemService) {
    DBManager = dbManager;
...
//    return ResponseEntity.status(HttpStatus.NOT_FOUND).body("파일이 없음");}
  ...
  String Edit(@PathVariable long id, Model model) {
    model.addAttribute("item", DBManager.findById(id).get());
    return "editItem";
  }
  ...
  String editItem(@ModelAttribute Item item, RedirectAttributes redirectAttributes) {
    itemService.updateItem(item, redirectAttributes);
    return "redirect:/list";
  }
  ...
  ResponseEntity<String> deleteItem(@RequestParam long id) {
    ...
    if (DBManager.findById(id).isPresent()) {
  ...
  String addPost(String title, Integer price, RedirectAttributes redirectAttributes) {
    itemService.saveItem(title, price, redirectAttributes); //redirectAttributes 로 메세지도 전송
    return "redirect:/list";
  }
